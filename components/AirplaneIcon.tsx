...
const AirplaneIcon = ({ className = '', direction = 'right' }: AirplaneIconProps) => {
  const directionClass = direction === 'left' ? '-scale-x-100' : ''

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      className={`h-5 w-5 fill-current ${directionClass} ${className}`.trim()}
      aria-hidden
      focusable="false"
    >
